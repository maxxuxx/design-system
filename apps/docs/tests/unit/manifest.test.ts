...
describe('component manifest', () => {
  ...
  it('sorts all fifteen entries in canonical order regardless of file order', () => {
    ...
    expect(manifest.components.map(({ name }) => name))
      .toEqual(['Icon', 'Badge', 'Button', 'TextField', 'ScrollArea', 'Checkbox', 'RadioGroup', 'Switch', 'Textarea', 'Select', 'TextButton', 'IconButton', 'BoardRow', 'Tab', 'BottomSheet']);
  ...
  it('requires all entries before the release check can pass', () => {
    expect(() => buildComponentManifest([document('Icon')], { requireFigma: true }))
      .toThrow('Release manifest is missing components: Badge, Button, TextField, ScrollArea, Checkbox, RadioGroup, Switch, Textarea, Select, TextButton, IconButton, BoardRow, Tab, BottomSheet');
  });

  it('requires a Figma URL on every complete release entry', () => {
    const documents = ['Icon', 'Badge', 'Button', 'TextField', 'ScrollArea', 'Checkbox', 'RadioGroup', 'Switch', 'Textarea', 'Select', 'TextButton', 'IconButton', 'BoardRow', 'Tab', 'BottomSheet']
      .map((name) => document(name as ComponentMetadata['name']));
    expect(() => buildComponentManifest(documents, { requireFigma: true }))
      .toThrow('Figma URLs are required for release: Icon, Badge, Button, TextField, ScrollArea, Checkbox, RadioGroup, Switch, Textarea, Select, TextButton, IconButton, BoardRow, Tab, BottomSheet');
  });
