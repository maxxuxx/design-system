...
props:
  ...
  - name: ...svgProps
    type: "Omit<SVGProps<SVGSVGElement>, 'children' | 'role' | 'aria-label' | 'aria-labelledby' | 'aria-hidden' | 'tabIndex' | 'focusable'>"
    required: false
    defaultValue: null
    description: Component-owned geometry, naming, visibility, and focus semantics are excluded; remaining native SVG props are forwarded.
tokens:
  - size/icon/small
  - size/icon/medium
  - size/icon/large
...
| native SVG props | <code>Omit&lt;SVGProps&lt;SVGSVGElement&gt;, 'children' \| 'role' \| 'aria-label' \| 'aria-labelledby' \| 'aria-hidden' \| 'tabIndex' \| 'focusable'&gt;</code> | none | Props outside component-owned geometry, naming, visibility, and focus semantics |

`label` owns the accessible name. Callers cannot override `role`, `aria-label`, `aria-labelledby`, `aria-hidden`, `tabIndex`, `focusable`, or the fixed geometry. Keep Icon decorative when visible text already names the action, as in the example above.
