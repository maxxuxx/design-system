...
props:
  ...
  - name: type
    ...
    required: false
    defaultValue: text
    description: text, email, password, search, tel, url 중 하나이며 다른 값은 runtime에서 text로 정규화됩니다.
  - name: ...inputProps
    type: "Omit<InputHTMLAttributes<HTMLInputElement>, 'size' | 'type'>"
    required: false
    defaultValue: null
    description: numeric HTML size와 unrestricted native type을 제외한 native input props입니다.
tokens:
  - size/control/medium
  - size/control/large
  - space/2
...
import { TextField } from '@hds/react';
...
| `type` | `'text' \| 'email' \| 'password' \| 'search' \| 'tel' \| 'url'` | `'text'` | supported single-line input type; unsupported runtime values normalize to text |
| native input props | <code>Omit&lt;InputHTMLAttributes&lt;HTMLInputElement&gt;, 'size' \| 'type'&gt;</code> | 없음 | controlled/uncontrolled/native behavior |
